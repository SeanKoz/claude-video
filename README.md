...
5. **Frames + transcript are handed to Claude.** The script prints frame paths with `t=MM:SS` markers and the transcript with timestamps. It also saves `summary.md` in the same working directory. Claude `Read`s each frame in parallel — JPEGs render directly as images in its context.
