...
7. **Summary on disk.** The same answer Claude shows the user is written to `summary.md` in that work directory via `scripts/save_summary.py` (stdin), as documented in `SKILL.md` — not generated by `watch.py`.
8. **Cleanup.** The script prints a working directory at the end. If you're not asking follow-ups, Claude removes it.
