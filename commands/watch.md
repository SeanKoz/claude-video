...
Follow the skill's full pipeline: preflight setup check → download via yt-dlp → extract frames at auto-scaled fps → pull captions or Whisper transcript → `watch.py` saves transcript markdown in the working directory (`yt-<videoId>.md` for YouTube `v=...`, else `transcript.md`) → Read each frame → answer the user grounded in frames and transcript → run `save_summary.py` with stdin to write the exact user-facing answer to `summary.md` in the same work directory (no confirmation prompt). If the user provided no arguments, ask them for a video URL or local path before proceeding.
