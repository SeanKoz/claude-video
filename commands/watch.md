...
Follow the skill's full pipeline: preflight setup check → download via yt-dlp → extract frames at auto-scaled fps → pull captions or Whisper transcript → save transcript markdown in the working directory (`yt-<videoId>.md` for YouTube `v=...`, else `transcript.md`) and save `summary.md` in the same folder → Read each frame → answer the user grounded in frames and transcript. If the user provided no arguments, ask them for a video URL or local path before proceeding.
